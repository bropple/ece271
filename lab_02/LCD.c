...
void LCD_PIN_Init(void){
	...
	//ST's given headers were not working as intended, so the next best thing was to make my own.
	...
	GPIOA->MODER &= ~(GPIO_MODER_CUST0); //Clear GPIOA bits with inverse of 0xC03FF000
	GPIOA->MODER |= (GPIO_MODER_CUST0_1); //Set bits
	
	GPIOA->AFR[0] &= ~(GPIO_AFRL_CUST3); //Clear GPIOA AFR with the inverse of 0xFF000000
	GPIOA->AFR[0] |= (GPIO_AFRL_CUST0); //Set bits
	
	GPIOA->AFR[1] &= ~(GPIO_AFRH_CUST0_1); //Clear bits with inverse of 0xF0000FFF
	GPIOA->AFR[1] |= (GPIO_AFRH_CUST0); //Set bits
	
	GPIOB->MODER &= ~(GPIO_MODER_CUST1); //Clear bits with inverse of 0xFF0C0F0F
	GPIOB->MODER |= (GPIO_MODER_CUST1_1); //Set bits
	
	GPIOB->AFR[0] &= ~(GPIO_AFRL_CUST1_1); //Clear bits with inverse of 0x00FF00FF
	GPIOB->AFR[0] |= (GPIO_AFRL_CUST1);
	
	GPIOB->AFR[1] &= ~(GPIO_AFRH_CUST1_1); //Clear bits with inverse of 0xFFFF00F0
	GPIOB->AFR[1] |= (GPIO_AFRH_CUST1);
	
	GPIOC->MODER &= ~(GPIO_MODER_CUST3); //Clear bits with inverse of 0x000FFFFF
	GPIOC->MODER |= (GPIO_MODER_CUST3_1);
	
	GPIOC->AFR[0] &= ~(GPIO_AFRL_CUST2_1); //Clear bits with inverse of 0xFFFFF000
	GPIOC->AFR[0] |= (GPIO_AFRL_CUST2);
	
	GPIOC->AFR[1] &= ~(GPIO_AFRH_AFSEL8); //Clear bits with inverse of 0X0000000F
	GPIOC->AFR[1] |= (GPIO_AFRH_CUST2);
	
	GPIOD->MODER &= ~(GPIO_MODER_CUST4); //Clear bits with inverse of 0xFFFF0000
	GPIOD->MODER |= (GPIO_MODER_CUST4_1);
	
	GPIOD->AFR[0] &= ~(GPIO_AFRL_CUST3); //Clear bits with the inverse of 0xFFFFFFFF
	GPIOD->AFR[0] |= (0UL);
	
	GPIOD->AFR[1] &= ~(GPIO_AFRH_CUST3_1); //Clear bits with inverse of 0xFFFFFFFF
	GPIOD->AFR[1] |= (GPIO_AFRH_CUST3);
}
...
void LCD_DisplayString(uint8_t* ptr){
	while(1) {
		...
		{
			int i = 0;  //Plays animation quickly, alternating + and *
			while(i<4) 
			{
				WaitUPD();
				LCD_Clear();
				WaitVRAM();
				for(cnt = 0; cnt<6; cnt++) {
					LCD_WriteChar(&((uint8_t*)"*+*+*+")[cnt], 0, 0, cnt);
				}
				Delay(2);
				WaitUPD();
				LCD_Clear();
		...
		{
			int i = 0;  //Plays animation, but shows each character being written to the screen.
			while(i<4) 
			{
				WaitUPD();
				LCD_Clear();
